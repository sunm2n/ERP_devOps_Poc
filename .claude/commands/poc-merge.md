...
**리뷰의 `report` 항목을 `docs/findings.md` 에 적재한다 — 머지 전에.**
3관점이 매 라운드 report 를 내는데 옮기는 단계가 루프에 없으면 아무도 안 옮기고,
보고서 5·7장이 4단계 마지막 밤에 기억으로 채워진다. 형식은 `docs/findings.md` 상단 참조 —
출처 / ADR / 내용 / 영향 / 권고. **도입권고 분류는 권고 문장으로 끝나야 한다.**
가설이 판정됐으면 판정 현황표도 갱신한다 (`미착수` / `참` / `거짓` / `판정불가` 4종만).
